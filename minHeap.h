...
vector<string> MinHeap::getTopWithinGenreHelper(vector<string> condensedGenres) {
    ...
    while (animeList.size() < 5)
    {
        ...
        for (int i = 0; i < top.genres.size(); i++)
        {
            ...
            for (int k = 0; k < condensedGenres.size(); k++) {
                if (allAnime[index].genres[i] == condensedGenres[k])
                {
                    animeList.push_back(allAnime[index].title);
                    alreadyAdded = true;
    ...
    //todo fix this so it's extract min
    while (animeList.size() < 5)
    {
        //iterate through genres in each anime obj
        for (int i = 0; i < allAnime[index].genres.size(); i++)
        {
            //iterate through the genres in condensedGenres
            for (int k = 0; k < condensedGenres.size(); k++) {
                if (allAnime[index].genres[i] == condensedGenres[k])
